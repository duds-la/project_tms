...
class CarrierSearchByNameService
{
    public function searchCarrierByNameOnDb(string $name): Collection
    {
        ...
        $search = DB::table('deliveries')
            ...
            ->leftJoin('trackings', 'deliveries.tracking_id', '=', 'trackings.id')
            ->join('carriers', 'deliveries.carrier_uuid', '=', 'carriers.uuid')
            ->where('carriers.company_name', 'like', '%' . $name . '%')
            ->get();

        if ($search->isEmpty()) {
            throw ValidationException::withMessages(['Transportadora Não encontrada, por favor verifique o nome inserido!']);   
        }
